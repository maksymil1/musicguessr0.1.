...
export default function Solo() {
  ...
  const fetchTrack = async () => {
    ...
    try {
      ...
      if (!track) return alert("Brak nowych, oryginalnych utworów! Zmień tryb.");

      setTargetTrack({
        id: track.trackId,
        name: track.trackName,
        cleanName: track.trackName.split("(")[0].split("[")[0].split("feat")[0].trim(),
  ...
  const handleGuess = (selectedTitle?: string) => {
    ...
    const isCorrect = normGuess === normFull || normGuess === normClean || normFull.startsWith(normGuess);
        ...
        {gameState === "setup" ? (
          <div className="setup-section">
            <div className="category-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "20px" }}>
              <button onClick={() => setCategory("popular")} className={category === "popular" ? "btn-active" : "btn-off"}>TOP HITS</button>
              <button onClick={() => setCategory("artist")} className={category === "artist" ? "btn-active" : "btn-off"}>ARTIST</button>
            </div>
            {category === "artist" && (
              <input type="text" placeholder="Wpisz artystę..." value={subValue} onChange={(e) => setSubValue(e.target.value)} className="invite-form-input" />
            )}
          ...
          <div className="game-container">
            ...
            <div className="history-list" style={{ marginBottom: "20px" }}>
              {[...Array(LEVELS.length)].map((_, idx) => (
                <div key={idx} className="history-item" style={{ height: "35px", border: "1px solid #333", marginBottom: "5px", display: "flex", alignItems: "center", padding: "0 10px", fontSize: "0.8rem", background: attemptsHistory[idx] ? "rgba(255, 71, 87, 0.15)" : "transparent", color: attemptsHistory[idx] ? "#ff4757" : "#555" }}>
                  {attemptsHistory[idx] || (idx === currentAttempt ? "TWOJA PRÓBA..." : "")}
                </div>
              ))}
            </div>
            ...
            <div style={{ position: "relative", marginTop: "20px" }}>
              <input type="text" placeholder="Wpisz tytuł..." value={guess} onChange={(e) => setGuess(e.target.value)} autoComplete="off" style={{ width: "100%", padding: "12px", background: "#111", border: "1px solid #444", color: "#fff" }} onKeyDown={(e) => e.key === "Enter" && handleGuess()} />
